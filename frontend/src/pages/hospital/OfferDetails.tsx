  ...
  const fetchOfferDetails = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        // Fetch real applicants for this offer
        try {
          const appsRes = await applicationService.getReceivedApplications({ offer_id: id });
          if (appsRes.success && appsRes.data) {
            const mapped: Applicant[] = (appsRes.data as Application[]).map((app) => ({
              id: app.id,
              name: `${app.first_name || ""} ${app.last_name || ""}`.trim(),
              university: app.faculty || app.hospital_name || "",
              status: app.status,
              gpa: "-",
              avatar: app.avatar,
            }));
          ...
          console.warn("Could not fetch applicants for offer", appsError);
          setApplicants([]);
        }
                      ...
                      <div key={applicant.id} className="flex items-center justify-between p-4 bg-muted/50 rounded-lg">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => navigate(`/hospital/applications/${applicant.id}`)}
                          >
                            View Details
                          </Button>
                        </div>
              ...
              <CardContent>
                <div className="space-y-6">
                  ...
                  <div className="flex items-start gap-4">
                    <div className={`w-3 h-3 rounded-full mt-1.5 ${offer.status === 'published' ? 'bg-green-500' :
                  ...
                  {/* Application period */}
                  <div className="flex items-start gap-4">
                    <div className={`w-3 h-3 rounded-full mt-1.5 ${!offer.application_deadline || new Date(offer.application_deadline) > new Date()
                      ? 'bg-amber-500'
                      : 'bg-gray-400'
                      }`} />
                    <div>
                  ...
                  {/* Internship period */}
                  <div className="flex items-start gap-4">
                    <div className={`w-3 h-3 rounded-full mt-1.5 ${new Date(offer.start_date) > new Date() ? 'bg-blue-500' : 'bg-green-500'
                      }`} />
                    <div>
