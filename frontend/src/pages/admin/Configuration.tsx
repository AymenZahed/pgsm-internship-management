...
export default function AdminConfiguration() {
  ...
  const fetchConfiguration = async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        // General
        setGeneralSettings({
          platformName: config.platformName || generalSettings.platformName,
          supportEmail: config.supportEmail || generalSettings.supportEmail,
          defaultLanguage: config.defaultLanguage || generalSettings.defaultLanguage,
          timezone: config.timezone || generalSettings.timezone,
          maintenanceMode: config.maintenanceMode === 'true',
        });
        // Security
        setSecuritySettings({
          sessionTimeout: config.sessionTimeout || securitySettings.sessionTimeout,
          maxLoginAttempts: config.maxLoginAttempts || securitySettings.maxLoginAttempts,
          passwordMinLength: config.passwordMinLength || securitySettings.passwordMinLength,
          requireEmailVerification: config.requireEmailVerification === 'true',
          twoFactorEnabled: config.twoFactorEnabled === 'true',
        });
        // Internship
        setInternshipSettings({
          maxApplicationsPerStudent: config.maxApplicationsPerStudent || internshipSettings.maxApplicationsPerStudent,
          applicationDeadlineDays: config.applicationDeadlineDays || internshipSettings.applicationDeadlineDays,
          autoAssignTutor: config.autoAssignTutor === 'true',
          requireDocuments: config.requireDocuments === 'true', // Default to true if not present? careful logic
          minAttendanceRate: config.minAttendanceRate || internshipSettings.minAttendanceRate,
        });
      }
    } catch (err: any) {
  ...
  const handleSave = async (section: string) => {
    try {
      ...
      let data = {};
      if (section === 'General') {
        data = { ...generalSettings, maintenanceMode: String(generalSettings.maintenanceMode) };
      } else if (section === 'Security') {
        data = { ...securitySettings, requireEmailVerification: String(securitySettings.requireEmailVerification), twoFactorEnabled: String(securitySettings.twoFactorEnabled) };
      } else if (section === 'Internship') {
        data = { ...internshipSettings, autoAssignTutor: String(internshipSettings.autoAssignTutor), requireDocuments: String(internshipSettings.requireDocuments) };
      }
